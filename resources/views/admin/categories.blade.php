...
<div class="period-theme-wrap">
  <div class="period-theme-shell">
    <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap mb-3">
      <div>
        <h1 class="h5 period-title">Categories</h1>
        <div class="period-subtitle">Manage category master data for assessment configuration.</div>
      </div>
...
async function load(){
  const j = await odFetch('/api/adm/categories');
  tblBody.innerHTML = '';
  (j.data||[]).forEach(r => {
...
document.getElementById('btnSave').addEventListener('click', async ()=>{
  ...
  try{
    await odFetch('/api/adm/category', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)});
    mdl.hide();
    await load();
  }catch(err){ odToast(err.message); }
});
