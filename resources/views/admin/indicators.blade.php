...
<div class="period-theme-wrap">
  <div class="period-theme-shell">
    <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap mb-3">
      <div>
        <h1 class="h5 period-title">Indicators</h1>
        <div class="period-subtitle">Manage indicator master data for assessment configuration.</div>
      </div>
...
<div class="modal fade period-dialog" id="mdl" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title">Indicator</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body">
        <input type="hidden" id="id">
        <div class="mb-2"><label class="form-label">Prefix</label><input class="form-control" id="prefix" maxlength="3"></div>
        <div class="mb-2"><label class="form-label">Title</label><textarea class="form-control" id="title" maxlength="150" rows="4"></textarea></div>
...
async function load(){
  const j = await odFetch('/api/adm/indicators');
  tblBody.innerHTML = '';
  (j.data||[]).forEach(r => {
...
tblBody.addEventListener('click', async (e)=>{
  ...
  if(act==='del'){
    const confirmed = await odConfirm('Delete this indicator?', 'Delete Indicator');
    if(!confirmed) return;
    try{ await odFetch(`/api/adm/indicator/${id}`, {method:'DELETE'}); await load(); }
...
document.getElementById('btnSave').addEventListener('click', async ()=>{
  ...
  try{
    await odFetch('/api/adm/indicator', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)});
    mdl.hide();
    await load();
  }catch(err){ odToast(err.message); }
});
